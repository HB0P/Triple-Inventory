...
public enum ShulkerPosition {
    ...
    BOTTOM((left, width) -> left + width / 2 - 88, (top, height) -> top + height - 11, (width, height) -> width == 176 ? "bottom_fill" : "bottom_middle", true),
    ...
    TOP((left, width) -> left + width / 2 - 88, (top, height) -> top - 68, (width, height) -> width == 176 ? "top_fill" : "top_middle"),
    ...
    public Identifier getTexture(int width, int height, int extendedInventorySize) {
        if (useExtendedWidth && extendedInventorySize != 0) {
            width += (extendedInventorySize * 36 + 8);
        }
